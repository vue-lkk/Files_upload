        ...
        try {
            ...
            data = await instance.post('/upload_single', formData, {
                ...
                onUploadProgress(ev) {
                    ...
                    upload_progress_value.style.innerHTML = `${loaded/total*100}%`;
                }
            });
            if (+data.code === 0) {
            ...
            upload_progress_value.style.innerHTML = `0%`;
        }
    });
...
(function () {
    ...
    upload_inp.addEventListener('change', async function (event) {
        let file = upload_inp.files[0];
        if (!file) return;
        upload_button_select.classList.add('loading');
        upload_progress.style.display = 'block';
        ...
        // 实现文件切片处理 「固定数量 & 固定大小」
        let max = 1024 * 1024 * 2, // 先固定大小
            count = Math.ceil(file.size / max), // 计算切片数量
            index = 0, // 计数
            chunks = []; //切片数据

        if (count > 100) {
            max = file.size / 100; // 修改大小
            count = 100;  // 固定切片数量
        }
        // 切割，将所有的切片对象存储到chunks数组
        while (index < count) {
            chunks.push({
        ...
        const complate = async () => {
            ...
            // upload_progress_value.innerHTML = `${(index/count*100).toFixed(0)}%`
            // // 当所有切片都上传成功，我们合并切片
            // // if (index < count) return;
            // // 进度条100%
            // upload_progress_value.style.width = `100%`;
            // upload_progress_value.innerHTML = `100%`
            // return
        ...
        // （发送请求）遍历把每一个切片都上传到服务器上
        let _files=[]
        _files = chunks.map(chunk => {
            // 已经上传的无需再上传
            ...
            upload_progress_value.style.width = `${index/count*100}%`;
            upload_progress_value.innerHTML = `${(index/count*100).toFixed(0)}%`
            console.log( upload_progress_value.innerHTML )
           
           return instance.post('/upload_chunk', fm)
    
            // instance.post('/upload_chunk', fm).then(data => {
            //     alert.log(data.codeText)
            //     if (+data.code === 0) {
            //         // complate();
            //         return;
            //     }
        ...
        Promise.all(_files).then((data) => {
            // alert(`恭喜您，所有文件都上传成功~~${data[0].codeText}`);
            if(data[0].code === 0) {
                complate()
            }
        }).catch(() => {
            ...
            _files = [];
            // upload_progress_value.style.width = `0%`;
            // upload_progress_value.innerHTML = ``
        });
    });
